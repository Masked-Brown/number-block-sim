...
const champion = getAgent('expectimax-d3-v1');
...
for (const [id, run] of Object.entries(evalRuns)) {
  ...
  const s = read(file);
  ladder[id] = { median: s.score.median, q1: s.score.q1, q3: s.score.q3, max: s.score.max, run: run.split('/')[0] };
}
...
const knowledge = {
  ...
  job: 'orchestrated-training-campaign',
  champion: {
    id: champion.id,
